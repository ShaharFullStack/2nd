 ...
 */
function PostureBeat({ lane, change, reducedMotion }: { lane: LaneSpec; change: PostureChange; reducedMotion: boolean }) {
 ...
 */
function ReuseBeat({ lane, offer, reducedMotion }: { lane: LaneSpec; offer: ReuseOffer; reducedMotion: boolean }) {
  const info = MOVEMENT_INFO[lane.movement];
  return (
...
export function CalibrationGuide({
  lane, status, tracking, accepted, refused, reducedMotion, retry, beat = 'measure', postureChange, reuse, reused = false,
}: {
  ...
  if (beat === 'posture' && postureChange) return <PostureBeat lane={lane} change={postureChange} reducedMotion={reducedMotion} />;
  if (beat === 'reuse' && reuse) return <ReuseBeat lane={lane} offer={reuse} reducedMotion={reducedMotion} />;
  const info = MOVEMENT_INFO[lane.movement];
